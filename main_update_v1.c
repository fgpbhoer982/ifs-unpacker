	...
	*/
	...
	for (uint32_t i = 0; i < BetTable.EntryCount - 1; i++) {
		...
		if (chunk > 1) {
			...
			lseek(file, FilePosition + offset + 1, SEEK_SET);
			
			read(file, buffer, arr[0]);
			write(fd, buffer, arr[0]);
			
			for (uint32_t i = 1; i < chunk; i++) {
				size_t l = arr[i] - arr[i-1];
				read(file, buffer, l);
				...
				int len = ZLIB_decompress(buffer, l, decom, 65536);
				
				if (len == 0) {
					printf("Can't decompress!\n");
				} else {
					printf("Decompression successfully!\n");
				}
		...
		*/
		...
		if (lseek(file, FilePosition + offset + 1, SEEK_SET) == -1) {
			fprintf(stderr, "Can't seek!\n");
		}
		...
		uint32_t offset = FilePosition + offset + 1;
		
		while (remaining > 0) {
			int chunk_size = remaining > 65536 ? 65536 : remaining;
			if (pread(file, buffer, chunk_size, offset) == -1) {
				printf("Can't pread!\n");
			}
		...
		if (chunk > 1) {
			...
			for (uint32_t i = 0; i < chunk; i++) {
				int chunk_size = arr[i+1] - arr[i];
				
				printf("chunk size: %u\n", chunk_size);
				
				if (chunk_size <= 0) {
					printf("error chunk: %u\n", chunk_size);
					sleep(2);
					continue;
				}
			...
			read(file, buffer, CompressedSize);
			write(fd, buffer, CompressedSize);
		}
